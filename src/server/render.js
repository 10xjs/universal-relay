...
function render(context, currentIteration = 1) {
  const {relayEnvironment} = context;

  return new Promise((resolve) => {
    // Render the component tree to an HTML string. renderToString does not
    // execute the full component lifecycle, but does call componentWillMount.
    // This gives any Relay containers the opportunity to run queries, which are
    // stored in the context.
    const markup = renderToString(<Root relayEnvironment={relayEnvironment} />);

    // Retrieve and clear the stored promises from the context. This returns an
    // array of js promises representing each query that was run during the
    // current render iteration. Each promise will resolve in turn as each query
    // result is returned from the GraphQL endpoint.
    ...
    return Promise.all(promises).then(() => {
      resolve(render(context, currentIteration + 1));
    });
  });
}
    ...
    request(req, res) {
      ...
      render(context).then((markup) => {
        ...
        const page = renderToStaticMarkup(
          <Page markup={markup} state={state} />
        );
        const body = `<!DOCTYPE html>${page}`;
